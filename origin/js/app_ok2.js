...
var App = {
    ...
    tiltEvent: null,
    tiltInit: function () {
        var speed = 0, translateX=30, _count = 1, lastDir, originTime, nowDir, moveDir, minDir = 1.5, gapTime = (App.parseUA().android) ? 50 : 500;
        ...
        if(window.DeviceOrientationEvent) {
            App.tiltEvent = function(e) {
                ...
                if (parseInt(sep_Time) > gapTime) { // 每隔0.8s进行一次速度测试
                    //左右旋转使背景图移动
                    nowDir = e.alpha; //360°旋转
                    (_count==2) && (lastDir = nowDir);
                    if(_count>2) {
                        ...
                        if(Math.abs(moveDir)>minDir){
                            $("#test").html("moveDir: "+Math.floor(moveDir)+",now44Dir: "+Math.floor(nowDir)+",lastDir: "+Math.floor(lastDir));
